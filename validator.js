...
module.exports = {
	validate: function(error, coin, targetPrice, callback) {
		...
		Binance.checkPrice(null, coin.trim().toUpperCase(), 'ETH', function(currentPrice) {
			...
			else if ((targetPrice * (1 + 1.50)) < currentPrice && currentPrice < Config.maximumPrice) {
			...
			Logging.log(`validate(): validCoin: ${Config.validCoin}. validTargetPrice: ${Config.validTargetPrice}.`);
